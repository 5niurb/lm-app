...
if (smsIdx !== -1) {
  ...
  flow.states[smsIdx] = {
    ...
    transitions: [
      { event: 'success', next: 'play_MsgSentGoodbye' },
      { event: 'failed' }
    ]
  };
  console.log(`✓ Replaced send_message_accounts (was ${oldType}) with make-http-request → ${RENDER_URL}`);
} else {
...
if (goodbyeState) {
  ...
  goodbyeState.transitions = goodbyeState.transitions.map(t => {
    if (t.next === 'fcn_NewSMSEmailNotify') {
      console.log('✓ play_MsgSentGoodbye: removed transition to fcn_NewSMSEmailNotify (now terminal)');
