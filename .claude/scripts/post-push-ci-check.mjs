...
async function checkCI() {
	...
	while (Date.now() - startTime < MAX_WAIT) {
		try {
			...
			if (!runs.length) {
				console.error('[ci-check] No CI runs found — repo may not have workflows.');
				return;
			}

			// Find a run created after our push (with 5s tolerance for clock skew)
			const cutoff = new Date(pushTime.getTime() - 5000);
			const ourRun = runs.find((r) => new Date(r.createdAt) >= cutoff) || null;
			...
			if (ourRun.status === 'completed') {
				if (ourRun.conclusion === 'success') {
					console.error(`[ci-check] ✓ ${ourRun.name} PASSED (run #${trackedRunId}).`);
				} else {
					console.error(`\n[ci-check] ✗ ${ourRun.name} FAILED (run #${trackedRunId}).`);
					fetchFailedLogs(trackedRunId);
					console.error('[ci-check] ↑ Fix the issues above and push again.\n');
				}
				return;
			}
			...
			console.error(`[ci-check] ${ourRun.name}: ${ourRun.status}... (${elapsed}s)`);
		} catch (err) {
...
function fetchFailedLogs(runId) {
	try {
		...
		console.error(`[ci-check] Could not fetch failure logs: ${err.message}`);
	}
}
...
async function checkDeploy() {
	...
	if (!project) {
		console.error('[deploy-check] Could not determine project name from command — skipping.');
		return;
	}
	...
	for (let i = 0; i < MAX_RETRIES; i++) {
		try {
			...
			if (resp.ok) {
				console.error(`[deploy-check] ✓ Deployment healthy (HTTP ${resp.status}).`);
				return;
			}

			console.error(`[deploy-check] Got HTTP ${resp.status} — retrying in 5s...`);
		} catch (err) {
			console.error(`[deploy-check] Attempt ${i + 1}/${MAX_RETRIES} failed: ${err.message}`);
		}

		if (i < MAX_RETRIES - 1) await sleep(5000);
	}

	console.error(`[deploy-check] ✗ Deployment verification failed after ${MAX_RETRIES} attempts.`);
}
