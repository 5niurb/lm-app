 ...
 */
router.post('/sequences', requireAdmin, logAction('automation.sequences.create'), async (req, res) => {
  const {
    service_id, name, trigger_event, timing_offset,
    channel, template_type, content_ref, subject_line,
    message_body, rcs_actions, is_active, sort_order
  } = req.body;

  if (!name || !trigger_event || !timing_offset || !template_type) {
    return res.status(400).json({ error: 'name, trigger_event, timing_offset, and template_type are required' });
  }
 ...
 */
router.put('/sequences/:id', requireAdmin, logAction('automation.sequences.update'), async (req, res) => {
  const {
    service_id, name, trigger_event, timing_offset,
    channel, template_type, content_ref, subject_line,
    message_body, rcs_actions, is_active, sort_order
 ...
 */
router.delete('/sequences/:id', requireAdmin, logAction('automation.sequences.delete'), async (req, res) => {
  try {
 ...
 */
router.post('/sequences/reorder', requireAdmin, logAction('automation.sequences.reorder'), async (req, res) => {
