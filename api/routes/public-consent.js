...
router.post('/:slug/submit', async (req, res) => {
  try {
      ...
      if (existingContact) {
        ...
        const { data: newContact, error: contactErr } = await supabaseAdmin
          ...
          .insert({
            ...
            phone_normalized: phoneNorm ? (phoneNorm.startsWith('1') ? `+${phoneNorm}` : `+1${phoneNorm}`) : null,
            source: 'manual',
            patient_status: 'new',
            tags: ['consent-form'],
    ...
    // Log it
    console.log(`[consent] Submission ${submission.id} from client ${resolvedClientId} for form ${resolvedFormId}`);
