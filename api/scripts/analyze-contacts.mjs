...
while (true) {
	const { data } = await sb.from('contacts')
		.select('id, source, source_id, phone_normalized, full_name, first_name, last_name, tags, email')
		.range(from, from + 999);
...
for (const [phone, contacts] of dupes.slice(0, 10)) {
	...
	for (const c of contacts) {
		console.log('    ', c.source, '|', c.full_name || 'unnamed', '|', c.email || 'no email', '| tags:', (c.tags || []).join(','));
	}
}
