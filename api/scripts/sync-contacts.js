...
function mapRecord(raw) {
	...
	const firstName =
		raw['first name'] || raw['first_name'] || raw['firstname'] || raw['first'] || '';
	...
	const sourceId =
		raw['ar id'] || raw['id'] || raw['patient id'] || raw['patient_id'] || raw['contact id'] || '';
	const status =
		raw['status'] || raw['patient status'] || raw['patient_status'] || raw['contact type'] || '';
	...
	if (normalizedSource.includes('aesthetic') || normalizedSource.includes('ar')) {
		...
		!['aesthetic_record', 'gohighlevel', 'textmagic', 'manual'].includes(normalizedSource)
	) {
...
async function syncFromCsv(filePath) {
	...
	const contacts = records.map(mapRecord).filter((c) => c.full_name || c.phone || c.email); // Skip empty rows
...
async function syncFromGoogleSheet() {
	...
	if (!res.ok || res.headers.get('content-type')?.includes('html')) {
		...
		console.error(
			'  1. Publish the "patients3" tab: File → Share → Publish to web → patients3 tab → CSV'
		);
		console.error(
			'  2. Export as CSV manually and run: node api/scripts/sync-contacts.js --csv <file.csv>'
		);
	...
	const contacts = records.map(mapRecord).filter((c) => c.full_name || c.phone || c.email);
