...
if (!environment) {
  ...
  const envRes = await fetch(`https://serverless.twilio.com/v1/Services/${service.sid}/Environments`, {
    method: 'POST',
    headers: { 'Authorization': authHeader, 'Content-Type': 'application/x-www-form-urlencoded' },
    body: params.toString()
  });
...
for (const file of files) {
  ...
  if (assetRes.status === 409) {
    ...
    const existingAssets = await api(`https://serverless.twilio.com/v1/Services/${service.sid}/Assets?PageSize=100`);
...
while (buildStatus === 'building') {
  ...
  const statusRes = await api(`https://serverless.twilio.com/v1/Services/${service.sid}/Builds/${build.sid}`);
