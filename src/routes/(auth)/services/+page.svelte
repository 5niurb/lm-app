<script>
	...
	import { Sparkles, Plus, Pencil, Trash2, ChevronDown, ChevronRight, FileText, X, GripVertical, ArrowUp, ArrowDown } from '@lucide/svelte';
	...
	const contentTypes = [
		{ value: 'pre_instructions', label: 'Pre-Treatment Instructions', color: 'text-blue-400', shortLabel: 'Pre-Care' },
		{ value: 'post_instructions', label: 'Post-Treatment Instructions', color: 'text-green-400', shortLabel: 'Post-Care' },
		{ value: 'consent_form', label: 'Consent Form', color: 'text-yellow-400', shortLabel: 'Consent' },
		{ value: 'questionnaire', label: 'Questionnaire', color: 'text-purple-400', shortLabel: 'Questionnaire' },
		{ value: 'faq', label: 'FAQ', color: 'text-cyan-400', shortLabel: 'FAQ' },
		{ value: 'what_to_expect', label: 'What to Expect', color: 'text-orange-400', shortLabel: 'Expectations' }
	];
	...
	function openCreateForm() {
		editingService = null;
		formName = ''; formSlug = ''; formCategory = 'advanced_aesthetics';
		formDescription = ''; formDuration = ''; formPrice = ''; formActive = true;
		showForm = true;
	}

	function openEditForm(service) {
		editingService = service;
		formName = service.name; formSlug = service.slug; formCategory = service.category;
		formDescription = service.description || ''; formDuration = service.duration_min?.toString() || '';
		formPrice = service.price_from?.toString() || ''; formActive = service.is_active;
		showForm = true;
	}
	...
	async function saveService() {
		if (!formName || !formSlug || !formCategory) { showToast('Name, slug, and category are required', 'error'); return; }
		formSaving = true;
		try {
			const body = { name: formName, slug: formSlug, category: formCategory, description: formDescription || null,
				duration_min: formDuration ? parseInt(formDuration) : null, price_from: formPrice ? parseFloat(formPrice) : null, is_active: formActive };
			if (editingService) {
				await api(`/api/services/${editingService.id}`, { method: 'PUT', body: JSON.stringify(body) });
	...
	async function toggleExpanded(serviceId) {
		if (expandedId === serviceId) { expandedId = null; return; }
	...
	async function refreshContent(serviceId) {
		...
		} catch { serviceContent[serviceId] = []; }
	}
	...
	function openContentCreate(serviceId, serviceName) {
		...
		cType = 'pre_instructions'; cTitle = ''; cSummary = ''; cPageSlug = ''; cActive = true;
	...
	function openContentEdit(content, serviceName) {
		...
		cSections = Array.isArray(content.content_json) && content.content_json.length > 0
	...
	function addSection() { cSections = [...cSections, { heading: '', body: '' }]; }
	...
	async function saveContent() {
		if (!cType || !cTitle) { showToast('Content type and title are required', 'error'); return; }
		cSaving = true;
		try {
			// Filter out empty sections
			const sections = cSections.filter(s => s.heading.trim() || s.body.trim());
			const body = {
				content_type: cType, title: cTitle, summary: cSummary || null,
				page_slug: cPageSlug || null, content_json: sections, is_active: cActive
			};

			if (editingContent) {
				await api(`/api/services/content/${editingContent.id}`, { method: 'PUT', body: JSON.stringify(body) });
				showToast('Content updated');
			} else {
				await api(`/api/services/${contentServiceId}/content`, { method: 'POST', body: JSON.stringify(body) });
	...
	function contentTypeLabel(val) { return contentTypes.find(c => c.value === val)?.label || val; }
	function contentTypeColor(val) { return contentTypes.find(c => c.value === val)?.color || 'text-white/50'; }

	let grouped = $derived(
		categories.map(cat => ({ ...cat, services: services.filter(s => s.category === cat.value) }))
...
<div class="space-y-6">
	...
	{#if showContentEditor}
		<div class="fixed inset-0 z-40 bg-black/70 flex items-start justify-center pt-12 overflow-y-auto" role="dialog">
			<div class="w-full max-w-3xl mx-4 mb-12 rounded-lg border border-[rgba(197,165,90,0.2)] bg-[#0e0e10] shadow-2xl">
				<!-- Editor header -->
				<div class="flex items-center justify-between px-6 py-4 border-b border-[rgba(197,165,90,0.1)]">
					<div>
						<h2 class="text-base tracking-wide">{editingContent ? 'Edit Content' : 'New Content'}</h2>
						<p class="text-xs text-[rgba(255,255,255,0.3)] mt-0.5">for <span class="text-[#c5a55a]">{contentServiceName}</span></p>
					</div>
					<button onclick={() => showContentEditor = false} class="p-1 text-[rgba(255,255,255,0.3)] hover:text-white transition-colors">
						<X class="h-5 w-5" />
					</button>
				</div>
				...
				<div class="p-6 space-y-5">
					...
					<div class="grid gap-4 sm:grid-cols-2">
						<div>
							<label class="text-xs uppercase tracking-[0.12em] text-[rgba(255,255,255,0.4)] mb-1 block">Content Type *</label>
							<select bind:value={cType} onchange={autoContentSlug}
						...
						<div>
							<label class="text-xs uppercase tracking-[0.12em] text-[rgba(255,255,255,0.4)] mb-1 block">Title *</label>
							<input type="text" bind:value={cTitle} placeholder="e.g. Botox Pre-Treatment Guide"
								class="w-full px-3 py-2 rounded border border-[rgba(255,255,255,0.1)] bg-[rgba(255,255,255,0.03)] text-sm focus:border-[#c5a55a] focus:outline-none transition-colors" />
						</div>
					</div>
					...
					<div>
						<label class="text-xs uppercase tracking-[0.12em] text-[rgba(255,255,255,0.4)] mb-1 block">
							SMS/Email Summary
							<span class="normal-case tracking-normal text-[rgba(255,255,255,0.2)]">— 2-3 sentences sent in messages, links to full page</span>
						</label>
						<textarea bind:value={cSummary} rows="2" placeholder="Brief summary for text messages..."
					...
					<div class="grid gap-4 sm:grid-cols-2">
						<div>
							<label class="text-xs uppercase tracking-[0.12em] text-[rgba(255,255,255,0.4)] mb-1 block">
								Page Slug
								<span class="normal-case tracking-normal text-[rgba(255,255,255,0.2)]">— lemedspa.com/care/</span>
							</label>
							<input type="text" bind:value={cPageSlug} placeholder="e.g. botox-pre"
					...
					<div>
						<div class="flex items-center justify-between mb-3">
							<label class="text-xs uppercase tracking-[0.12em] text-[rgba(255,255,255,0.4)]">
								Content Sections
								<span class="normal-case tracking-normal text-[rgba(255,255,255,0.2)]">— expandable accordion on care page</span>
							</label>
						...
						<div class="space-y-3">
							{#each cSections as section, idx}
								<div class="rounded border border-[rgba(255,255,255,0.06)] bg-[rgba(255,255,255,0.01)] p-4">
									<div class="flex items-center gap-2 mb-2">
										<span class="text-[10px] text-[rgba(255,255,255,0.2)] w-5 text-center">{idx + 1}</span>

										<input type="text" bind:value={section.heading} placeholder="Section heading..."
										...
										<div class="flex items-center gap-0.5">
											<button onclick={() => moveSection(idx, -1)} disabled={idx === 0}
												class="p-1 text-[rgba(255,255,255,0.15)] hover:text-[rgba(255,255,255,0.5)] disabled:opacity-20 transition-colors" title="Move up">
												<ArrowUp class="h-3 w-3" />
											</button>
											<button onclick={() => moveSection(idx, 1)} disabled={idx === cSections.length - 1}
												class="p-1 text-[rgba(255,255,255,0.15)] hover:text-[rgba(255,255,255,0.5)] disabled:opacity-20 transition-colors" title="Move down">
												<ArrowDown class="h-3 w-3" />
											</button>
											<button onclick={() => removeSection(idx)} disabled={cSections.length <= 1}
												class="p-1 text-[rgba(255,255,255,0.15)] hover:text-red-400 disabled:opacity-20 transition-colors" title="Remove">
												<Trash2 class="h-3 w-3" />
											</button>
										</div>
									</div>

									<textarea bind:value={section.body} rows="4"
										placeholder="Section body — detailed instructions, restrictions, expectations..."
										class="w-full px-2 py-1.5 rounded border border-[rgba(255,255,255,0.06)] bg-transparent text-xs leading-relaxed focus:border-[#c5a55a] focus:outline-none transition-colors resize-y"></textarea>
								</div>
							{/each}
						</div>
					</div>
				...
				<!-- Editor footer -->
				<div class="flex items-center justify-between px-6 py-4 border-t border-[rgba(197,165,90,0.1)]">
					<div class="text-xs text-[rgba(255,255,255,0.2)]">
						{cSections.filter(s => s.heading.trim() || s.body.trim()).length} section{cSections.filter(s => s.heading.trim() || s.body.trim()).length !== 1 ? 's' : ''}
						{#if editingContent}· v{editingContent.version}{/if}
					</div>
					<div class="flex items-center gap-3">
						<button onclick={() => showContentEditor = false}
							class="px-4 py-2 text-sm text-[rgba(255,255,255,0.5)] hover:text-white transition-colors">Cancel</button>
						<button onclick={saveContent} disabled={cSaving}
	...
	{#if showForm}
		<div class="rounded border border-[rgba(197,165,90,0.2)] bg-[rgba(197,165,90,0.03)] p-6 space-y-4">
			<div class="flex items-center justify-between">
				...
				<button onclick={() => showForm = false} class="text-[rgba(255,255,255,0.3)] hover:text-white transition-colors"><X class="h-4 w-4" /></button>
			</div>
			<div class="grid gap-4 sm:grid-cols-2">
				<div>
					<label class="text-xs uppercase tracking-[0.12em] text-[rgba(255,255,255,0.4)] mb-1 block">Name *</label>
					<input type="text" bind:value={formName} oninput={autoSlug} placeholder="e.g. Neuromodulators (Botox/Dysport)"
						class="w-full px-3 py-2 rounded border border-[rgba(255,255,255,0.1)] bg-[rgba(255,255,255,0.03)] text-sm focus:border-[#c5a55a] focus:outline-none transition-colors" />
				</div>
				<div>
					<label class="text-xs uppercase tracking-[0.12em] text-[rgba(255,255,255,0.4)] mb-1 block">Slug *</label>
					<input type="text" bind:value={formSlug} placeholder="e.g. neuromodulators"
						class="w-full px-3 py-2 rounded border border-[rgba(255,255,255,0.1)] bg-[rgba(255,255,255,0.03)] text-sm font-mono focus:border-[#c5a55a] focus:outline-none transition-colors" />
				</div>
				<div>
				...
				<div class="flex gap-4">
					<div class="flex-1">
						...
						<input type="number" bind:value={formDuration} placeholder="45"
							class="w-full px-3 py-2 rounded border border-[rgba(255,255,255,0.1)] bg-[rgba(255,255,255,0.03)] text-sm focus:border-[#c5a55a] focus:outline-none transition-colors" />
					</div>
					<div class="flex-1">
						<label class="text-xs uppercase tracking-[0.12em] text-[rgba(255,255,255,0.4)] mb-1 block">Price From ($)</label>
						<input type="number" bind:value={formPrice} placeholder="250" step="0.01"
							class="w-full px-3 py-2 rounded border border-[rgba(255,255,255,0.1)] bg-[rgba(255,255,255,0.03)] text-sm focus:border-[#c5a55a] focus:outline-none transition-colors" />
					</div>
				</div>
			</div>
			<div>
				...
				<textarea bind:value={formDescription} rows="2" placeholder="Brief service description..."
					class="w-full px-3 py-2 rounded border border-[rgba(255,255,255,0.1)] bg-[rgba(255,255,255,0.03)] text-sm focus:border-[#c5a55a] focus:outline-none transition-colors resize-none"></textarea>
			</div>
			<div class="flex items-center gap-6">
				...
				<button onclick={saveService} disabled={formSaving}
	...
	{:else}
		{#each grouped as group}
			<div>
				<div class="flex items-center gap-2 mb-3">
					<span class="px-2 py-0.5 rounded text-[10px] uppercase tracking-[0.12em] font-medium {categoryColor(group.value)}">{group.label}</span>
						...
						<div class="rounded border border-[rgba(197,165,90,0.08)] hover:border-[rgba(197,165,90,0.15)] transition-all duration-200 bg-[rgba(255,255,255,0.01)] hover:bg-[rgba(197,165,90,0.02)] {group.value === 'advanced_aesthetics' ? 'border-l-2 border-l-[rgba(197,165,90,0.3)]' : group.value === 'regenerative_wellness' ? 'border-l-2 border-l-emerald-500/30' : 'border-l-2 border-l-purple-500/30'}">
							...
							<div class="flex items-center gap-3 px-4 py-3">
								<button onclick={() => toggleExpanded(service.id)} class="text-[rgba(255,255,255,0.25)] hover:text-[#c5a55a] transition-colors">
									{#if expandedId === service.id}<ChevronDown class="h-4 w-4" />{:else}<ChevronRight class="h-4 w-4" />{/if}
								</button>
								<div class="flex-1 min-w-0">
									<div class="flex items-center gap-2">
										<span class="text-sm font-medium text-[rgba(255,255,255,0.85)]">{service.name}</span>
										{#if !service.is_active}<Badge variant="outline" class="text-[10px]">Inactive</Badge>{/if}
									</div>
									{#if service.description}<p class="text-xs text-[rgba(255,255,255,0.3)] mt-0.5 truncate">{service.description}</p>{/if}
								</div>
								<div class="flex items-center gap-4 text-xs text-[rgba(255,255,255,0.3)]">
									...
									{#if service.price_from}<span>From ${Number(service.price_from).toFixed(0)}</span>{/if}
									<span class="font-mono text-[rgba(255,255,255,0.15)]">/{service.slug}</span>
								</div>
								{#if $isAdmin}
									<div class="flex items-center gap-1">
										<button onclick={() => openEditForm(service)}
											class="p-1.5 rounded text-[rgba(255,255,255,0.2)] hover:text-[#c5a55a] hover:bg-[rgba(197,165,90,0.05)] transition-colors" title="Edit">
											<Pencil class="h-3.5 w-3.5" />
										</button>
										<button onclick={() => deleteService(service)}
											class="p-1.5 rounded text-[rgba(255,255,255,0.2)] hover:text-red-400 hover:bg-red-500/5 transition-colors" title="Delete">
								...
								<div class="px-4 pb-4 pt-1 border-t border-[rgba(197,165,90,0.06)]">
									<div class="ml-7">
										<div class="flex items-center justify-between mb-3">
											...
											{#if $isAdmin}
												<button onclick={() => openContentCreate(service.id, service.name)}
										...
										{#if serviceContent[service.id]?.length > 0}
											<div class="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
												{#each serviceContent[service.id] as content}
													<div class="group rounded border border-[rgba(255,255,255,0.06)] px-3 py-2.5 bg-[rgba(255,255,255,0.01)] hover:border-[rgba(197,165,90,0.15)] transition-colors cursor-pointer"
														role="button" tabindex="0"
														onclick={() => openContentEdit(content, service.name)}
														onkeydown={(e) => { if (e.key === 'Enter') openContentEdit(content, service.name); }}>
														<div class="flex items-center justify-between">
															<div class="flex items-center gap-2">
															...
															{#if $isAdmin}
																<button onclick={(e) => { e.stopPropagation(); deleteContent(content); }}
																	class="p-0.5 opacity-0 group-hover:opacity-100 text-[rgba(255,255,255,0.15)] hover:text-red-400 transition-all" title="Delete">
																	<Trash2 class="h-3 w-3" />
																</button>
															{/if}
														</div>
														<div class="flex items-center gap-2 mt-1">
															...
															{#if Array.isArray(content.content_json)}
																<span class="text-[10px] text-[rgba(255,255,255,0.12)]">{content.content_json.length} section{content.content_json.length !== 1 ? 's' : ''}</span>
															{/if}
															{#if content.page_slug}
																<span class="text-[10px] text-[rgba(255,255,255,0.12)] font-mono">/care/{content.page_slug}</span>
															{/if}
														</div>
														{#if content.summary}
										...
										<div class="mt-3 flex flex-wrap gap-1.5">
											{#each contentTypes as ct}
												...
												<button
													...
													onclick={() => { if (!exists && $isAdmin) { cType = ct.value; openContentCreate(service.id, service.name); } }}
													title={exists ? `${ct.label} exists — click to edit` : `Add ${ct.label}`}
												>
													{exists ? '✓' : '+'} {ct.shortLabel}
												</button>
											{/each}
										</div>
									</div>
		...
		<div class="flex items-center gap-6 pt-4 border-t border-[rgba(197,165,90,0.06)]">
			<span class="text-xs text-[rgba(255,255,255,0.25)]">{services.length} service{services.length !== 1 ? 's' : ''}</span>
			<span class="text-xs text-[rgba(255,255,255,0.25)]">{services.filter(s => s.is_active).length} active</span>
		</div>
	{/if}
</div>
