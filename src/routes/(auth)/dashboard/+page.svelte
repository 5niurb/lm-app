<script>
	...
	import { Phone, PhoneMissed, Voicemail, Clock, PhoneIncoming, PhoneOutgoing, MessageSquare, ArrowRight, PhoneCall } from '@lucide/svelte';
	...
	// Sparkline helpers
	let sparkTotalMax = $derived(dailyStats && dailyStats.length > 0 ? Math.max(1, ...dailyStats.map(d => d.total)) : 1);
	let sparkMissedMax = $derived(dailyStats && dailyStats.length > 0 ? Math.max(1, ...dailyStats.map(d => d.missed)) : 1);
...
<div class="space-y-8">
	...
	<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
		<div class="group card-elevated rounded border p-5 transition-all duration-200 relative overflow-hidden">
			...
			{#if stats}
				...
				{#if dailyStats && dailyStats.length > 0}
					<svg class="w-full h-8 mt-2 opacity-40 group-hover:opacity-70 transition-opacity" viewBox="0 0 {dailyStats.length * 20} 30" preserveAspectRatio="none">
						<defs><linearGradient id="spark-gold" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="#C5A55A" stop-opacity="0.3"/><stop offset="100%" stop-color="#C5A55A" stop-opacity="0"/></linearGradient></defs>
						<polyline fill="none" stroke="#C5A55A" stroke-width="1.5" stroke-linejoin="round" points="{sparkPoints(dailyStats, 'total', sparkTotalMax)}" />
						<polygon fill="url(#spark-gold)" points="{sparkFill(dailyStats, 'total', sparkTotalMax)}" />
					</svg>
				{/if}
			{:else}
		...
		<div class="group card-elevated rounded border p-5 transition-all duration-200 relative overflow-hidden">
			...
			{#if stats}
				...
				{#if dailyStats && dailyStats.length > 0}
					<svg class="w-full h-8 mt-2 opacity-40 group-hover:opacity-70 transition-opacity" viewBox="0 0 {dailyStats.length * 20} 30" preserveAspectRatio="none">
						<defs><linearGradient id="spark-red" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="#f87171" stop-opacity="0.3"/><stop offset="100%" stop-color="#f87171" stop-opacity="0"/></linearGradient></defs>
						<polyline fill="none" stroke="#f87171" stroke-width="1.5" stroke-linejoin="round" points="{sparkPoints(dailyStats, 'missed', sparkMissedMax)}" />
						<polygon fill="url(#spark-red)" points="{sparkFill(dailyStats, 'missed', sparkMissedMax)}" />
					</svg>
				{/if}
			{:else}
				<Skeleton class="h-9 w-16" />
				...
				{:else}
					<div class="flex h-48 items-center justify-center">
						<div class="text-center">
							<div class="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-xl bg-[rgba(197,165,90,0.05)] border border-[rgba(197,165,90,0.08)]">
								<Phone class="h-5 w-5 empty-state-icon" />
							</div>
							<p class="text-sm text-[rgba(255,255,255,0.3)]">No call data yet</p>
							<p class="text-xs text-[rgba(255,255,255,0.15)] mt-1">Charts will populate once calls are logged.</p>
						</div>
					</div>
				{/if}
			</div>
			...
			{:else if recentCalls.length === 0}
				<div class="flex h-40 items-center justify-center">
					<div class="text-center">
						...
						<p class="text-sm font-light text-[rgba(255,255,255,0.35)]" style="font-family: 'Playfair Display', serif;">No calls yet</p>
							...
							<div class="min-w-0 flex-1">
								<div class="flex items-center justify-between gap-2">
									...
									<div class="flex items-center gap-1.5 shrink-0">
										...
										{/if}
										<span class="text-xs text-[rgba(255,255,255,0.3)]">{formatRelativeDate(call.started_at)}</span>
									</div>
								</div>
								<div class="mt-0.5">
