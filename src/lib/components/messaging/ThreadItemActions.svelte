...
<div
	class="flex flex-col items-center gap-0.5 transition-opacity {isStarred || isResolved
		? 'opacity-100'
		: 'opacity-0 group-hover:opacity-100'}"
>
	<button
		type="button"
		class="flex h-8 w-8 items-center justify-center rounded-md transition-colors {isStarred
	...
	<button
		type="button"
		class="flex h-8 w-8 items-center justify-center rounded-md transition-colors {isResolved
	...
	>
		<CircleCheck class="h-4 w-4" fill={isResolved ? 'currentColor' : 'none'} />
	</button>
</div>
